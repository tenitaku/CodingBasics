...
/*
 ＜ポイント＞
 ・ビット演算は、2つの配列の集合比較の際にとても便利。
 ・今回のように、add関数を定義して使用することで、数字を含むかどうかのビット配列を作成することができる。
 ・ビット配列から数字に変換する際には、i~Nまでfor文でbit列.test(i)とし、、新配列cont.push_back(i)とすれば良い。
 ・出力の際には、新配列cont.size()分ループで取り出していけば良い。
 */
